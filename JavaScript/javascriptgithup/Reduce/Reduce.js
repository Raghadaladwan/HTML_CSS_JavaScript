...
{
var out;
var Ag=persons.reduce((x, y) => {
    return out= x+y.age;
},0);

console.log(out/persons.length);
}
avgAge(persons);
...
function longestName(arr){
    var long= arr.reduce((x,y)=>{
        if(x.length<(y.name.first+y.name.values).length){
...
{   
    console.log(arr.filter(x=>x%4==0));
}
multiFour([1,8,6,4]);
/*
8
...
{
 console.log( str.filter((x, y)=>y%2==0 && x.length%2==1));
...
function olderThan(arr,age){
  console.log(arr.filter(x => x.age > age));
}
...
function shorterThan(arr,num){
  console.log(arr.filter(x => x.length < num));
}
